        ...
        .forEach(function (data) {
            ...
            if (regPathInstructions.test(data)) {
                ...
                if (instruction == "A" || instruction == "a") {
                    ...
                    for (
                        var args;
                        (args = regArcArgumentSequence.exec(data));
...
exports.applyTransforms = function (elem, path, params) {
    ...
    if (
        ...
        elem.someAttr(function (attr) {
            return (
                ~referencesProps.indexOf(attr.name) &&
                ~attr.value.indexOf("url(")
            );
        })
    )
        return path;
        ...
        if (scale !== 1) {
            ...
            if (
                ...
                if (elem.hasAttr("stroke-width")) {
                    elem.attrs["stroke-width"].value = elem.attrs[
                        "stroke-width"
                    ].value
                        .trim()
                    ...
                    elem.addAttr({
                        ...
                        prefix: "",
                        local: "stroke-width",
                        value: strokeWidth.replace(regNumericValues, function (
                            num
                        ) {
                            return removeLeadingZero(num * scale);
                        }),
